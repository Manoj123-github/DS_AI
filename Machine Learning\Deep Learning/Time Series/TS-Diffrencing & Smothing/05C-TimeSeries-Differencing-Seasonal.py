...
print("*** Differencing Seasonality ***")
ds = df - df.shift(12)
ds.dropna(inplace=True)

# Augmented Dickey-Fuller Test
print('\n*** Augmented Dickey-Fuller Test ***')
print(tsutils.adfTest(ds))

# plot
print("\n*** Plot Time Series - ds ***")
plt.figure(figsize=(9,4))
sns.lineplot(x=df.index, y=vColData, data=df, label='df', color='b')
sns.lineplot(x=ds.index, y=vColData, data=ds, label='ds', color='c')
plt.xticks(rotation=60)
plt.title('Differencing Seasonality')
plt.legend()
plt.show()
