...
df['TESA'] = ExponentialSmoothing(df['Passengers'],trend='add',seasonal='add',seasonal_periods=12).fit().fittedvalues
# triple exponentail smoothing - multiplicative
df['TESM'] = ExponentialSmoothing(df['Passengers'],trend='mul',seasonal='mul',seasonal_periods=12).fit().fittedvalues
...
dfaTest = df.iloc[len(df)-12:] # set one year(12 months) for dfTesting 
print("Done ...")
...
dfTrain = df.iloc[:len(df)-12] 
dfmTest = df.iloc[len(df)-12:] # set one year(12 months) for dfTesting 
print("Done ...")
...
# why 24 - 12 for dfTest & 12 for predict
lForecast = model.forecast(len(dfTest)+len(dfPred))
# why -12 - last 12 will be data from 12 months of predict
dfPred['Forecast'] = lForecast[-12:]
